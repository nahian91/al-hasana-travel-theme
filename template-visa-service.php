...
<section class="destination-details-section fix section-padding">
    <div class="container">
        <div class="destination-details-wrapper">
            ...
            <?php
            // Get all visa-service categories
            $visa_categories = get_terms(array(
                'taxonomy'   => 'visa_service_category',
                'hide_empty' => false,
            ));

            // Sort categories so "Asia" comes first
            $sorted_categories = array();
            if ( ! empty( $visa_categories ) && ! is_wp_error( $visa_categories ) ) {
                foreach ( $visa_categories as $key => $cat ) {
                    if ( strtolower($cat->name) === 'asia' ) {
                        array_unshift($sorted_categories, $cat); // Put Asia first
                    } else {
            ...
            if ( ! empty( $sorted_categories ) ) {
                foreach ( $sorted_categories as $visa_cat ) {
                    ?>
                    ...
                    <?php 
                    // Query visa-service posts for this category
                    $args = array(
                    ...
                    <div class="row visa-services-row">
                        ...
                        if ( $visa_query->have_posts() ) {
                            ...
                            echo '<p class="text-center">No Visa Services found in this category.</p>';
                        }
                        wp_reset_postdata();
                        ?>
                    </div>
                ...
                echo '<p class="text-center">No Visa Service Categories found.</p>';
            }
            ?>
        </div>
    </div>
